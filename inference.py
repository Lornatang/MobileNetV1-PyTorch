...
def build_model(model_arch_name: str, model_num_classes: int, device: torch.device) -> [nn.Module, nn.Module]:
    mobilenet_v1_model = model.__dict__[model_arch_name](num_classes=model_num_classes)
...
def main():
    ...
    mobilenet_v1_model = build_model(args.model_arch_name, args.model_num_classes, device)
    print(f"Build `{args.model_arch_name}` model successfully.")

    # Load model weights
    mobilenet_v1_model, _, _, _, _, _ = load_state_dict(mobilenet_v1_model, args.model_weights_path)
    ...
    # Inference
    with torch.no_grad():
        output = mobilenet_v1_model(tensor)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_arch_name", type=str, default="mobilenet_v1")
    ...
    parser.add_argument("--model_weights_path", type=str, default="./results/pretrained_models/MobileNetV1-ImageNet_1K.pth.tar")
